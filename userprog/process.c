...
process_execute (const char *args) 
{
  ...
  argsCopy = palloc_get_page (0);
  if (argsCopy == NULL) return TID_ERROR;
  strlcpy (argsCopy, args, PGSIZE);
  set_arg (args, file_name);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create(file_name, PRI_DEFAULT, start_process, argsCopy);

  if (tid == TID_ERROR)
    palloc_free_page (argsCopy);
  
  ct = thread_current ();

  p = initialize_process(tid);
  ...
  // add process to current thread
  list_push_back (&ct->childrenList, &p->elem);

  sema_down (&p->load);
  if (p->load_status==LOAD_FAILED) return -1;
...
start_process (void *args_)
{
  ...
  // parent us updated with load status
  update_parent_status (ct, success ? LOAD_SUCCESS : LOAD_FAILED);
...
int process_wait (tid_t child_tid){
  ...
  int exitcode;
  struct thread *ct = thread_current ();
  struct process *cp = get_child (ct, child_tid);
  
  if (cp == NULL) return -1;
  if (cp->waited) return -1;
  cp->waited = true;
  sema_down (&cp->wait);
...
process_exit (void)
{
  ...
  if (!list_empty (&ct->childrenList)) release_child (ct);
  if (!list_empty (&ct->fileList)) close_files (ct);
  if (ct->executedFile != NULL) {
      file_allow_write (ct->executedFile);
      file_close (ct->executedFile);
  }
...
load (const char *args, void (**eip) (void), void **esp)
{
  ...
  if (!add_values (esp, args)) goto done;
...
bool esp_move(void **esp, size_t size) {
  ...
  if (size > (uint32_t)PHYS_BASE) return false;
...
bool add_args (void **esp, const char *args) {
  ...
  while (current >= args){
    ...
    while (current >= args && *current != ' ') current--;
    start = current + 1;
    length = (end - start) + 1;
    if (!esp_move (&espTemp, length)) return false;
    strlcpy (espTemp, start, length);
  }

  align = ((uint32_t) espTemp) % 4;
  if (!esp_move (&espTemp, align)) return false;
...
bool add_argv(void **start, void **esp, int *argc) {
  ...
  if (!esp_move (&espTemp, sizeof (char *))) return false;
  *((char **) espTemp) = 0;
  while (current >= end)
    {
...
bool add_values (void **esp, const char *args) {
  void *espTemp = *esp;
  int argc = 0;
  if (!add_args (&espTemp, args)) return false;
  if (!add_argv (esp, &espTemp, &argc)) return false;
  if (!esp_move (&espTemp, sizeof (int))) return false;
  *((int *) espTemp) = argc;
  if (!esp_move (&espTemp, sizeof (int))) return false;
  *((int *) espTemp) = 0;
  *esp = espTemp;
...
void release_child(struct thread *t) {
  ...
  while (e != list_end (&t->childrenList)){
      ...
      e = list_next (e);
      if (p->alive) detach_parent (p->pid);
      list_remove (&p->elem);
      free (p);
    }
}
